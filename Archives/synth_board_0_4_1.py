...
import synth_layout_0_7 as synth_layout
...
# Are we actually going to load the sounds?
loadSounds = True
# Dan Posluns made a little loader status counter
displayLoadingProgress = True
# By default, no crossfade and a 1-sec out
fadeTime = 1000
crossFade = False

# Define the hotkeys
hotkeyFunctions = {
    "PLAY": "self.master_buttons[master.layout_hotkeys[\"%s\"]].queue()",
...
class MyFrame(wx.Frame):
    ...
    def __init__(self, parent, ID, title):
        ...
        self.statusBar.SetStatusWidths([-1,-1,-1,140])
        ...
        ###########################################
        self.global_events = {}
        self.global_events[wx.WXK_ESCAPE] = "self.cancel(disableFade=True)"
        ...
        for key in self.global_events:
            shortcut_table.append( (wx.ACCEL_NORMAL, key, key) )
        ...
        for hotkey in master.layout_hotkeys:
            ...
            # Build a hotkeyID as a combination of modifiers and the key itself
    ...
    def initialize(self):
        # Set our default mode by forcing the changemode
    ...
    def parseEvent(self,type):
    	# Its making beeps, maybe this is where?
        if type in self.global_events.keys():
    ...
    def toggleCrossfadeEnabled(self):
        global crossFade
        crossFade = not crossFade
        self.updateFadeDisplay()
    
    # Change the fade speed
    def setFadeTime(self, time):
        global fadeTime
        fadeTime = int(time * 1000)
        self.updateFadeDisplay()

    # This updates the status bar display of our fade settings
    def updateFadeDisplay(self):
        self.statusBar.SetStatusText("Fade time: %gs" % (float(fadeTime) / 1000), 1)
        if crossFade:
            s = "ENABLED"
        else:
            s = "DISABLED"
        self.statusBar.SetStatusText("Crossfade " + s, 2)
    ...
    def changeMode(self, firstrun=False):
        ...
        if len(self.modes) > 1 or firstrun:
            ...
            self.statusBar.SetStatusText("["+master.config["type"].upper()+"]  "+self.modes[self.mode].upper(), 3)
...
class MasterQueue():
    ...
    def __init__(self, wxFrame):
        ...
        self.loopSound = None
        self.loopChannel = None
        self.loopTimer = wx.Timer(wxFrame)
        wxFrame.Bind(wx.EVT_TIMER, self.LoopTimerCallback, self.loopTimer)
    ...
    def LoopTimerCallback(self, event):
        if self.loopChannel and self.loopChannel.get_queue() == None:
            self.loopChannel.queue(self.loopSound)
    
    def LoopSound(self, channel, sound):
        self.loopChannel = channel
        self.loopSound = sound
        self.loopTimer.Start(500, False)
    
    def CancelLooping(self):
        if self.loopChannel:
            self.loopChannel.queue(self.silence)
        self.loopTimer.Stop()
        self.loopSound = None
        self.loopChannel = None
...
class ContentGroup(wx.Panel):
    ...
    def __init__(self, parent, ID,name,column):
        ...
        for tag in column:
            ...
            button = FX_Button(self, -1, tag[0], tag[1], tag[2], tag[3])
...
class FX_Button(wx.ToggleButton):
    ...
    def __init__(self, parent, ID, name,src,loop=False,loopFile=None):
    ...
    def loadmode(self):
        ...
        for source in self.src:
            ...
            if not os.path.isfile(soundfile):
                print "Warning: sound file not found: " + source
                sound = master.frame.master_queue.silence
            else:
                if loadSounds:
                    sound = pygame.mixer.Sound(soundfile)
                    if displayLoadingProgress:
                        print "%d%%: %s" % (int(float(master.totalLoaded * 100) / master.totalSounds), source)
                else:
                    sound = None
        ...
        if self.loopFile:
            ...
            if not os.path.isfile(loopFilePath):
                ...
                if loadSounds:
                    self.loopSound = pygame.mixer.Sound(pathRoot+"/"+self.loopFile)
                else:
                    self.loopSound = None
        else:
        	self.loopSound = None
    ...
    def play(self,queueChannel=None):
        ...
        if not dontplay:
            ...
            if loadSounds and not master.testmode:
                if queueChannel:
                    self.channel = queueChannel
                    queueChannel.queue(self.sounds[playnum])
                    if self.loopSound:
                        master.frame.master_queue.LoopSound(self.channel, self.loopSound)
                    elif self.loop < 0:
                        master.frame.master_queue.LoopSound(self.channel, self.sounds[playnum])
                else:
                    if self.loopSound:
                        self.channel = self.sounds[playnum].play()
                        master.frame.master_queue.LoopSound(self.channel, self.loopSound)
                    else:
...
if __name__ == "__main__":
    createMaster("Projects\\FinalFantasysynt\\")