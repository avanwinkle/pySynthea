...
##   -- (0.9.2) Support for VLC, single and group playback, tooltips
...
def loadLayout(docroot="",file="Layout"):
    ...
    for line in openDatafile(docroot+file):
        ...
        if not line[0] or line[0][0] == "#":
            continue
        # Get the name of the page (or use the previous)
        pageName = line[0] or pageName
        # Get the name of the frame, made unique to the page name
        frameName = pageName + '/$/$' + line[1] if line[1] else frameName
        ...
        buttonSrc = [filename.strip() for filename in line[3].strip().split(",")]
        
        # Assume that we won't be looping the file, unless the loop flag is found
        buttonLoop = False
        loopFile = None
        buttonBuff = False
        buttonExclusive = False
        buttonTip = None
        ...
        if len(line) > 4:
            ...
            for l4val in l4vals:
                ...
                #     buttonBuff = 3		
                if l4val == "SINGLE":
                    buttonExclusive = buttonSrc
                elif l4val in ['DIALOG','MUSIC']:
                    buttonExclusive = l4val
                elif l4val.startswith("GROUP="):
                    buttonExclusive = l4val[6:]

        # Is there a tooltip?
        if len(line) > 5:
            buttonTip = line[5] or None

        # If we're missing key info, skip it
        if not buttonName or not buttonSrc:
            continue
        ...
        if not frameName in buttonorder.keys():
            ...
            validCount += 1
        # Or, if this button doesn't already exist in the frame, add it!
...
def loadHotkeys(docroot="",file="Hotkeys",functions=None):
    ...
    for key in openDatafile(docroot+file):
        try:
            ...
            #     if match is None:
            #         logger.debug("Error: %s is not a valid hotkey combination. Skipping hotkey entry." % key[0])
            #     elif not key[1] in functions:
            #         logger.debug("Error: %s is not a valid hotkey function. Skipping hotkey entry." % key[1])
            #     else:
...
def loadConfig(docroot="",file="Config.txt"):
    ...
    if config.has_key("dj") and config["dj"] == "enabled":
        ...
        config["dj"] = False
    if config.has_key('playback'):
        if config['playback'] == "vlc":
            config['playback'] = "vlc"
        elif config['playback'] == "pygame":
            config['playback'] = "pygame"
    else:
        # Set default
        config['playback'] = "pygame"
