...
import synth_board_0_4_5 as synth_board
...
# # OPTIONS = {
# #     'argv_emulation': True,
# #     'iconfile': 'Resources/synthea_icon_apple.icns'}
...
class ProjectFrame(wx.Frame):
    ...
    def __init__(self,parent,ID,title):
        ...
        if sys.platform == "darwin":
            # MenuBar = wx.MenuBar()
            # FileMenu = wx.Menu()
            # # Add an option to reload the layout
            # litem = FileMenu.Append(wx.ID_EXIT, "&Reload Layout")
            # self.Bind(wx.EVT_MENU, self.OnCloseWindow, litem)
            # # Add a quit to the file menu
            ...
            # This creates a taskbar icon, which isn't useful right now
            # self.tbicon = wx.TaskBarIcon()
            # icon = wx.Icon("Resources/synthea_icon_cropped.png",wx.BITMAP_TYPE_ANY)
    ...
    def reloadLayout(self,Event):
        log.debug("Reload layout!")

        syntheaBoard.destroy()
        syntheaBoard.exit()

        loadedButton.open(None)
...
class PJ_Button(wx.ToggleButton):
    ...
    def open(self,evt):
        ...
        loadedBoard = self
        syntheaBoard = synth_board.createMaster("Projects/"+self.path+"/",testmode)
